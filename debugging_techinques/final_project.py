...
def get_same_or_newer(start_date, parsed_data):
...
def parse_data(data):
    # Order and return the provided data
    reader = csv.reader(data[1:])
    sorted_data = sorted(reader, key=operator.itemgetter(3), reverse=False)
    return sorted_data
...
def list_newer(start_date):
    ...
    while start_date < datetime.datetime.today():
        start_date, employees = get_same_or_newer(start_date, parsed_data)
